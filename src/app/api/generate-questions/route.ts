...
async function generateQuestions(position: string, description: string, amount: number, type: string) {
  try {
    const prompt = `You are an expert interviewer. Generate a list of ${amount} high-quality ${type} interview questions for the following position.
      ...
      The questions should be short (no more than two sentences) and concise, so users are not frustrated and want to continue. Return ONLY the questions as a numbered list in plain text. Do NOT use any Markdown, formatting, or explanations. The question should be answerable in 90 seconds or less.`;
